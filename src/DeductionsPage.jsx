...
export default function DeductionsPage({ role }) {
  ...
  const [form, setForm] = useState({
    ...
    amount: "",
    note: "",
    deduct_date: new Date().toISOString().slice(0, 10),
  });
  ...
  const handleSave = async () => {
    ...
    if (error) {
      ...
      setForm(f => ({ ...f, amount: "", note: "" })); // เคลียร์แค่ amount/note ให้เลือกคนต่อไปได้เร็ว
      loadDeductions();
    }
    setSaving(false);
  };
  ...
  // ── group by พนักงาน เพื่อแสดงยอดรวม ──
      ...
      <div style={s.card}>
        ...
        {showAddType && (
          <div style={s.addTypeBox}>
            ...
            <button onClick={handleAddType} disabled={addingType} style={{ ...s.btn, ...s.btnPrimary }}>
              {addingType ? "⏳" : "บันทึก"}
            </button>
            ...
            {rows.map(row => (
              <div key={row.id} style={s.deductRow}>
                <div style={{ flex:1 }}>
                  ...
                  <span style={{ marginLeft:8, fontWeight:600 }}>{fmt(row.amount)} บาท</span>
                  {row.note && <span style={{ marginLeft:8, color:"#64748b", fontSize:12 }}>— {row.note}</span>}
                  <span style={{ marginLeft:8, color:"#94a3b8", fontSize:11 }}>{row.deduct_date}</span>
                </div>
                {canEdit(row) && (
        ...
        {deductions.filter(d => d.is_paid).map(row => (
          <div key={row.id} style={{ ...s.deductRow, opacity:0.65 }}>
            <div style={{ flex:1 }}>
              ...
              <span style={s.typeBadge}>{row.deduction_types?.name}</span>
              <span style={{ marginLeft:6, fontWeight:600 }}>{fmt(row.amount)} บาท</span>
              {row.note && <span style={{ marginLeft:6, color:"#64748b", fontSize:12 }}>— {row.note}</span>}
              <span style={{ marginLeft:6, color:"#94a3b8", fontSize:11 }}>{row.deduct_date}</span>
            </div>
            <span style={s.lockedBadge}>🔒 หักแล้ว</span>
            {role === "owner" && (
...
const s = {
  ...
  typeBadge: { marginLeft:6, background:"#eff6ff", color:"#1d4ed8",
    padding:"1px 8px", borderRadius:12, fontSize:11, fontWeight:600 },
  lockedBadge: { fontSize:11, color:"#9ca3af", background:"#f1f5f9",
